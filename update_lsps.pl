...
sub main {
	...
	my $lsps           = $plan->get_table('<LSPs>');
	my $lsp_paths      = $plan->get_table('<LSPPaths>');
	my $ActualPathHops = $plan->get_table('<ActualPathHops>');

## --Update the PathOption column from LSPPaths into ActualPathHops:

## UPDATE ActualPathHops SET PathOption = (SELECT PathOption FROM LSPPaths WHERE LSPPaths.Source = ActualPathHops.Source AND LSPPaths.LSP = ActualPathHops.LSP);
	foreach my $actual_path_hop ($ActualPathHops->rows()) {
		my ($lsp_path) = $lsp_paths->get_rows_where({
			Source => $actual_path_hop->get_value('Source'),
			LSP    => $actual_path_hop->get_value('LSP'),
		});
		$actual_path_hop->set_value( PathOption => $lsp_path->get_value('PathOption') );
	}
## UPDATE LSPs SET NetIntAdminstatus = 'up', NetIntOperStatus = 'up';
	foreach my $lsp ($lsps->rows()) {
		$lsp->set_values({
