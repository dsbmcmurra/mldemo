...
sub main {
	...
	my $lsps        = table_extract($args->{'plan-file'}, [qw(LSPs)])->get_table('<LSPs>') or fatal();
	my $lsp_traffic = $plan->get_table('<LSPTraffic>');
	$lsp_traffic->empty();

	foreach my $lsp ($lsps->rows()) {
		$lsp_traffic->append()->set_values({
			Name         => $lsp->get_value('Name'),
			Source       => $lsp->get_value('Source'),
			TrafficLevel => 'default',
			Queue        => $EMPTY_STRING,
			TraffMeas    => $lsp->get_value('TraffSim')
		});
	}
