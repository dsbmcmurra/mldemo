	...
	my $app = application(
		...
		app_flags(
			...
			optional_flags(
				...
				flag('cpu-max',      {data_type=>'integer', default=>'47'}),
				flag('ram-floor',    {data_type=>'integer', default=>'15'}),
				flag('ram-variance', {data_type=>'integer', default=>'60'}),
				flag('cpu2-divisor', {data_type=>'integer', default=>'10'}),
			),
		),
	) or fatal();
	my $args = read_command_line($app) or fatal();
	my $plan = read_plan_file($args->{'plan-file'}) or fatal();
	my $nodes = $plan->get_table('<Nodes>');
	$args->{'seed'} and srand $args->{'seed'};

	foreach my $node ($nodes->get_rows_where({NetIntRE0Mem=>undef})) {
		my $vendor = $node->get_value('Vendor') || 'none';
		my $cpu1   = int rand $args->{'cpu-max'};
		my $ram    = $args->{'ram-floor'} + int rand $args->{'ram-variance'};
		my $values = {
			NetIntRE0CPU5m => $cpu1,
			NetIntRE0Mem   => $ram,
		};
		if ($vendor =~ /juniper/xmsoi) {
			my $cpu2 = int $cpu1 / $args->{'cpu2-divisor'};
			$values->{'NetIntRE1CPU5m'} = $cpu2;
			$values->{'NetIntRE1Mem'}   = int rand $ram;
		}
