...
sub main {
	my $app = application(
		...
		app_flags(
			required_flags(
				flag('archive', {data_type=>'string'}),
				flag('archive-cmds',  {data_type=>'wrtfile'}),
				flag('snaptime',  {data_type=>'cariden_time'}),
			),
		),
	) or fatal();
	my $args = read_command_line($app) or fatal();
	my $archive_cmds_file = $args->{'archive-cmd'};
	my $archive_dir       = $args->{'archive'};
	my $timestamp         = $args->{'snaptime'};
	open(my $fh, '>>', $args->{'archive-cmd'}) or fatal();

	print $fh "mate_convert -plan-file $archive_dir/$timestamp.txt -out-file $archive_dir/$timestamp.pln;\n";
	print $fh "archive_insert -plan-file $archive_dir/$timestamp.pln -archive $archive_dir -time $timestamp -verbosity 40;\n";
	print $fh "/bin/rm $archive_dir/$timestamp.pln $archive_dir/$timestamp.txt;\n";
